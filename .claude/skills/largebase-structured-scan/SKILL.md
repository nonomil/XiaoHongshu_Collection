...
> **自动判断**：`extract` 完成后，Codex 读取 `extract-data.json` 的模块数：
> - 模块数 < 5 → **策略 A**（单 Codex）
> - 模块数 ≥ 5 且用户未明确反对 → **策略 C**（并行 Codex，加快速度）
> - Codex 不可用 → **策略 B**（Codex 兜底）
...
按 `extract-data.json` 的模块清单拆分 scope，Codex **同时**调起多个 Codex：

```text
模块清单：[src/core, src/api, src/ui, services/auth, services/payment, docs]
    ↓ Codex 按模块拆分（每个 Codex 负责 1-2 个模块）
...
**注意**：
- 策略 C 不要求 Codex 完全同时启动，Codex 可以顺序触发；等全部完成后再 merge
...
路由：
- 任务数 `=1` → `codex-workflow-complex.md`（可跳过其 Phase 0）
- 任务数 `>=2` → `codex-workflow-parallel.md`
- 缺陷修复主导 → `codex-workflow-debug.md`
