...
> 基于 Codex + Claude Code MCP 的高质量软件开发规范
...
sequenceDiagram
    participant H as 人类
    participant Codex as Claude Code
    participant CX as Codex MCP
    H->>Codex: 描述任务
    Codex->>Codex: Plan Mode：Linus三问\n列出改动文件
    Codex->>H: 确认改动范围
    H->>Codex: 确认
    Codex->>CX: 调用MCP执行代码生成
    CX-->>Codex: 返回代码 + threadId
    Codex->>Codex: 轮次1：自身审查\n逻辑/安全/YAGNI
    Codex->>CX: 轮次2：独立深度审查
    CX-->>Codex: 边界条件/长尾问题
    Codex->>H: Review结果汇总
    H->>Codex: 确认无问题
...
<text x="200" y="238" fill="#64748b" text-anchor="middle">Codex审查</text>
...
| Review工具组合 | Codex + Claude Code（Cursor不推荐）|
