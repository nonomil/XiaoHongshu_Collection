...
test('scrollMoreComments includes a nudge scroll strategy for infinite lists', async () => {
  let capturedExpression = '';
  const evaluate = async (expression) => {
    capturedExpression = expression;
    return { scrolled: true, scrolledRoot: true };
  ...
  assert.equal(result, true);
  assert.match(capturedExpression, /nudgeScroll/);
  assert.match(capturedExpression, /scrollCandidates/);
});
