...
test('result group can fill only that group links back into input box', async () => {
  const dom = buildDom();
  ...
  const fillButton = dom.window.document.querySelector('[data-group-key="AI"] [data-group-action="fill-links"]');
  ...
  assert.match(dom.window.document.getElementById('status-text').textContent, /已填入 AI 2 条链接/);
});
